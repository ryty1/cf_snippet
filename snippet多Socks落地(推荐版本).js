        ...
        try {
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                ...
                if (url.pathname === '/api/check-socks') {
                    return await handleSocksCheck(request);
                }
                if (url.pathname.toLowerCase().includes(`/${subPath}`)) {
                    return await handleSubscriptionRequest(request, T);
                }
            }
...
function parseWsPacketHeader(chunk, token) {
    ...
    if (formatIdentifier(new Uint8Array(chunk.slice(1, 17))) !== token) return { hasError: true, message: E2 };

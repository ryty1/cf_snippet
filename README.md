...
2. **设置 PW 为强密码（或留空禁用密码）**
   ...
   ```
3. **【可选】Github远程配置，不用可不配置，默认即可，需要配置按照下面[进阶配置](#step-1-🛠️[进阶配置] Github 远程配置，突破 Sinppets 32KB 大小限制，有效配置将`覆盖`脚本中的配置，`优先级`)操作**
...
## step-1 🛠️[进阶配置] Github 远程配置，突破 Sinppets 32KB 大小限制，有效配置将`覆盖`脚本中的配置，`优先级`！

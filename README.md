...
一套由 Cloudflare Workers 搭配 KV 存储管理 Snippets 配置的项目。通过 KV 存储实现配置的集中管理和实时同步。
...
| [shadowsocks](https://github.com/ryty1/cf_Snippets/blob/main/worker_kv%E7%89%88/shadowsocks.js) | Snippets (free) | Shadowsocks 订阅服务 |可分享模式|
...
- **优选域名管理**：添加、编辑、删除、禁用优选 CF 域名
- **SOCKS 代理管理**：添加、编辑、删除、禁用 SOCKS5 代理配置
- **拖拽排序**：支持拖拽调整配置顺序
- **批量操作**：支持全选、批量删除
...
4. **Snippets 限制**：Snippets 有一定的代码大小限制（32KB）

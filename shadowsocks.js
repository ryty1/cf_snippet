...
let subPath = 'link', proxyIP = 'ProxyIP.cmliussss.net';
...
async function handleSSRequest(request, cpi) {
    const url = new URL(request.url), socksIndex = parseInt(url.searchParams.get('socks') || '0');
    let parsedSocksConfig = null, useSocksProxy = false;
    ...
    const wssPair = new WebSocketPair(), [clientSock, serverSock] = Object.values(wssPair);
    serverSock.accept();
    let rcw = { socket: null };
    const earlyData = request.headers.get('sec-websocket-protocol') || '', readable = makeReadableStr(serverSock, earlyData);
    readable.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            if (iSTS(hostname)) throw new Error('Speedtest site is blocked');
...
function parseSSPacketHeader(chunk) {
    ...
    try {
        const view = new Uint8Array(chunk), addressType = view[0];
        ...
        switch (addressType) {
            ...
            case 4:
                addrLen = 16;
                const ipv6 = [], ipv6View = new DataView(chunk.slice(addrValIdx, addrValIdx + addrLen));
...
async function c2s5(proxyConfig, targetHost, targetPort, initialData) {
    const { host, port, username, password } = proxyConfig, connectHost = host.includes(':') && !host.startsWith('[') ? `[${host}]` : host, socket = connect({ hostname: connectHost, port: port }), writer = socket.writable.getWriter(), reader = socket.readable.getReader();
    try {
        ...
        if (selectedMethod === 0x02) {
            ...
            const userBytes = new TextEncoder().encode(username), passBytes = new TextEncoder().encode(password), authPacket = new Uint8Array(3 + userBytes.length + passBytes.length);
            authPacket[0] = 0x01;
        ...
        const hostBytes = new TextEncoder().encode(targetHost), connectPacket = new Uint8Array(7 + hostBytes.length);
...
async function c2h(proxyConfig, targetHost, targetPort, initialData) {
    const { host, port, username, password } = proxyConfig, socket = connect({ hostname: host, port: port }), writer = socket.writable.getWriter(), reader = socket.readable.getReader();
    try {
...
async function handleCheckSocks(request) {
    const url = new URL(request.url), idx = url.searchParams.get('index'), checkOne = async (socksConfig) => {

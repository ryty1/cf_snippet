...
function pPA(s) { if (!s) return null; s = s.trim(); if (s.startsWith('socks://') || s.startsWith('socks5://')) { try { const u = new URL(s.replace(/^socks/, 'socks5')); return { type: 'socks5', host: u.hostname, port: parseInt(u.port) || 1080, username: u.username ? decodeURIComponent(u.username) : '', password: u.password ? decodeURIComponent(u.password) : '' }; } catch (e) { return null; } } if (s.startsWith('http://') || s.startsWith('https://')) { try { const u = new URL(s); return { type: 'http', host: u.hostname, port: parseInt(u.port) || (s.startsWith('https://') ? 443 : 80), username: u.username ? decodeURIComponent(u.username) : '', password: u.password ? decodeURIComponent(u.password) : '' }; } catch (e) { return null; } } if (s.includes('@')) { const ai = s.lastIndexOf('@'), ap = s.substring(0, ai), hp = s.substring(ai + 1); let u = '', pw = ''; const ci = ap.indexOf(':'); if (ci > 0) { u = ap.substring(0, ci); pw = ap.substring(ci + 1); } let h = hp, p = 1080; const lci = hp.lastIndexOf(':'); if (lci > 0) { h = hp.substring(0, lci); const pn = parseInt(hp.substring(lci + 1), 10); if (!isNaN(pn) && pn > 0 && pn <= 65535) p = pn; } return { type: 'socks5', host: h, port: p, username: u, password: pw }; } if (s.startsWith('[')) { const cb = s.indexOf(']'); if (cb > 0) { const h = s.substring(1, cb), r = s.substring(cb + 1); if (r.startsWith(':')) { const p = parseInt(r.substring(1), 10); if (!isNaN(p) && p > 0 && p <= 65535) return { type: 'direct', host: h, port: p }; } return { type: 'direct', host: h, port: 443 }; } } const lci = s.lastIndexOf(':'); if (lci > 0) { const h = s.substring(0, lci), ps = s.substring(lci + 1), p = parseInt(ps, 10); if (!isNaN(p) && p > 0 && p <= 65535) return { type: 'direct', host: h, port: p }; } return { type: 'direct', host: s, port: 443 }; }
